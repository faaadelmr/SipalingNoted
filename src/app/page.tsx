...
export default function Home() {
  ...
  const handleRemoveLine = (lineIndex: number) => {
    ...
    const lines = currentTab.content.split('\n');
    // Prevent removing the last line if it's the only one
    if (lines.length <= 1) {
        handleLineChange(0, "");
        return;
    }
  ...
  return (
    <main className="container mx-auto p-4 md:p-8 font-headline">
      <header className="flex items-center gap-4 mb-6">
        ...
        <div>
          <h1 className="text-3xl font-bold text-foreground">SipintarNoted</h1>
          <p className="text-muted-foreground">Your simple and smart notes, organized.</p>
        </div>
      </header>
              ...
              <CardContent className="p-4 md:p-6">
                <div className="space-y-1">
                  {linesForCurrentTab.map((line, index) => (
                    <div key={`${tab.id}-${index}`} className="flex items-center group gap-2 rounded-md hover:bg-muted/50 transition-colors">
                      <Input
                        type="text"
                        value={line}
                        onChange={(e) => handleLineChange(index, e.target.value)}
                        placeholder="Type something..."
                        className="flex-grow bg-transparent border-none focus-visible:ring-0 focus-visible:ring-offset-0 px-2 h-10"
                      />
                      <div className="flex items-center opacity-0 group-hover:opacity-100 focus-within:opacity-100 transition-opacity pr-2">
